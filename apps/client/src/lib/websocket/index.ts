...
async function connect(set: WebSocketStoreSet, get: () => WebSocketStore) {
  try {
    ...
    let ws = new WebSocket(`wss://${location.host}/api/ws?access_token=${access_token}&instanceId=${instanceId}`);
    ...
    ws.onclose = (event: CloseEvent) => {
      console.log(`[DiscordD&D WS] WebSocket connection closed. Code: ${event.code} Reason: ${event.reason}`);
